# Biketerra Fitness Format (.bff) 🤗
A lightweight JSON format for storing routes or activities.

## Why?

`GPX` doesn't support activities. `TCX` and GPX are XML-based: lots of data repetition = larger filesizes.

`FIT` is a binary format. It's very efficient, but the data is difficult to access without special tools.

`BFF` aims to be the "sweet spot" between data efficiency and readability.

## Structure

| Prop | Type | Required | Purpose |
| :--- | :--- | :------- | :------ |
| name | text | Y | The route or activity title |
| meta.description | text || Additional details |
| meta.author | text || The author name |
| meta.{field} | mixed || Custom meta, e.g. `totalDistance`
| keys | array | Y | row schemas; `[[fieldNames], ...]` |
| rows | array | Y | row data; `rows[N][0]` is the schema index |
| events | array || event actions; `[[timestamp, eventType, extraMeta], ...]` |

## Event types

| Event Type | Purpose |
| :--------- | :------ |
| stop | Stop activity recording |
| lap | Split an activity into chunks |
| session | Separate activities |

## Data fields

| Field | Datatype | Unit |
| :---- | :------- | :--- |
| timestamp | int | Unix epoch (ms) |
| latitude | int | WGS 84 * 10^7 |
| longitude | int | WGS 84 * 10^7 |
| altitude | int | cm |
| distance | int | cm |
| speed | int | cm/s |
| grade | int | grade * 100 |
| power | int | watts |
| cadence | int | RPM |
| heartRate | int | BPM |

## Examples

### Route (minimal)

```json
{
    "name": "My route",
    "keys": [
        ["latitude", "longitude", "altitude"]
    ],
    "rows": [
        [0, 340522350, -1182436830, 8900],
        [0, 340525000, -1182440000, 9500]
    ]
}
```

### Activity (minimal)

```json
{
    "name": "My activity",
    "keys": [
        ["timestamp", "latitude", "longitude", "altitude", "distance", "speed", "grade", "power", "cadence", "heartRate"]
    ],
    "rows": [
        [0, 1727599260000, 340522350, -1182436830, 8900, 100000, 500, 150, 200, 90, 80],
        [0, 1727599320000, 340525000, -1182440000, 9500, 150000, 520, 180, 210, 95, 85],
        [0, 1727599560000, 340527000, -1182445000, 10000, 200000, 550, 200, 220, 100, 90]
    ]
}
```

### Activity (with a stop)

```json
{
    "name": "My activity",
    "events": [
        [1727599320000, "stop"]
    ],
    "keys": [
        ["timestamp", "latitude", "longitude", "altitude", "distance", "speed", "grade", "power", "cadence", "heartRate"]
    ],
    "rows": [
        [0, 1727599260000, 340522350, -1182436830, 8900, 100000, 500, 150, 200, 90, 80],
        [0, 1727599320000, 340525000, -1182440000, 9500, 150000, 520, 180, 210, 95, 85],
        [0, 1727599560000, 340527000, -1182445000, 10000, 200000, 550, 200, 220, 100, 90]
    ]
}
```

### Triathalon (advanced)

An activity split into 3 sessions (running, swimming, cycling). The cycling session is split into laps.

```json
{
    "name": "Triathlon Activity",
    "events": [
        [1727656800000, "session", {"sport": "running"}],
        [1727658900000, "session", {"sport": "swimming"}],
        [1727660700000, "session", {"sport": "cycling"}],
        [1727662500000, "lap"],
        [1727663400000, "lap"]
    ],
    "keys": [
        ["timestamp", "latitude", "longitude", "altitude", "distance", "speed", "grade", "cadence", "heartRate"],
        ["timestamp", "latitude", "longitude", "distance", "heartRate"],
        ["timestamp", "latitude", "longitude", "altitude", "distance", "speed", "grade", "power", "cadence", "heartRate"]
    ],
    "rows": [
        [0, 1727657100000, 515074000, -1278000, 1500, 50000, 450, 120, 80, 140],
        [0, 1727657400000, 515075000, -1279000, 2000, 100000, 480, 150, 85, 150],
        [1, 1727659200000, 515076000, -1280000, 10000, 130],
        [1, 1727659800000, 515077000, -1281000, 20000, 140],
        [2, 1727661600000, 515078000, -1282000, 3000, 300000, 2000, 100, 90, 145],
        [2, 1727662500000, 515079000, -1283000, 3500, 400000, 2200, 120, 95, 150],
        [2, 1727663400000, 515080000, -1284000, 4000, 500000, 2400, 140, 100, 155],
        [2, 1727664300000, 515081000, -1285000, 4500, 600000, 2600, 160, 105, 160]
    ]
}
```
